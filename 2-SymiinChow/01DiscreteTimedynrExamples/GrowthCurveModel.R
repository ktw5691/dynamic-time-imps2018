...
formula2 = list(
  Level ~ Level + Deltat * Slope, # Intercept (Level) is previous Level + D * S
  Slope ~ Slope # Constant Slope
)
...
printex(GCmodel, ParameterAs = GCmodel$'param.names', show = FALSE,
        printInit = TRUE, printRS = FALSE,
        outFile = "GrowthCurve.tex")
tinytex::latexmk("GrowthCurve.tex")
...
LV = data.frame(ID = d$subject, time = d$time, levelEst = LV[, 1],
                slopeEst = LV[, 2])
